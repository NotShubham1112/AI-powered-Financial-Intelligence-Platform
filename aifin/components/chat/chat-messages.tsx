...
export function ChatMessages({ messages, isLoading }: ChatMessagesProps) {
  ...
  return (
    <div ref={containerRef} className="flex-1 overflow-y-auto">
      <div className="mx-auto max-w-[1200px] space-y-0 px-3 py-4 md:px-6 md:py-6">
        {messages.length === 0 && !isLoading && <EmptyState />}
        {messages.map((message, index) => (
          <div key={message.id} className="border-b border-border/60 py-4 md:py-6 last:border-b-0">
            <ChatMessage
              message={message}
              isStreaming={
        ...
        {isLoading &&
          ...
          messages[messages.length - 1].role === "user" && (
            <div className="border-b border-border/60 py-4 md:py-6">
              <RuntimeStatusBar />
  ...
  return (
    <div className="py-10 md:py-16">
      <div className="mb-8 md:mb-10">
        <TerminalBadge className="mb-4 md:mb-6">Research Terminal</TerminalBadge>
        <h1 className={typography.h1 + " mb-3 md:mb-4 text-foreground text-2xl md:text-4xl"}>
          Institutional AI infrastructure
        </h1>
        <p className={typography.body + " max-w-[640px] text-muted-foreground text-sm md:text-base"}>
      ...
      <div className="grid grid-cols-1 gap-[1px] border border-border bg-border sm:grid-cols-2">
        {workflows.map((item) => (
          <div
            key={item.cmd}
            className="cursor-pointer bg-background p-4 md:p-5 transition-colors hover:bg-accent"
          >
